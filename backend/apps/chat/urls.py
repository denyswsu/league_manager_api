...
from .views import (JoinRoomView, LeaveRoomView, MessageListCreateAPIView,
                    RoomDetailViewSet, RoomListViewSet, RoomSearchViewSet)

urlpatterns = [
    path("rooms/", RoomListViewSet.as_view({"get": "list"}), name="room-list"),
    path(
        "rooms/<int:pk>/",
        RoomDetailViewSet.as_view({"get": "retrieve"}),
        name="room-detail",
    ),
